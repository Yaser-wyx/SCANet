...
from src.bricks.bricks import get_brick_canonical_mesh
...
def pymesh2trimesh(pytorch3d_mesh: Meshes):
    # 获取PyTorch3D mesh对象的顶点、面坐标，忽略纹理
    verts = pytorch3d_mesh.verts_packed().cpu().numpy()
    faces = pytorch3d_mesh.faces_packed().cpu().numpy()
...
from src.bricks.brick_info import Brick, CBrick
...
def get_brick_masks(bricks_mesh_list, mask_colors, idxs, cameras, render_size=1536, output_size=512):
    ...
    # [1, H, W, 3]
    image_shadeless = renderer_simple(mesh_all, cameras=cameras) # 耗时较长，且无法压缩
    brick_masks = []
    for k in idxs:
        ref_pixel = torch.tensor(mask_colors[k],device="cuda:0").reshape(1, 1, 1, -1)
...
from src.lego.utils.data_generation_utils import flatten_nested_list, map_nested_list, index_list
