...
from src.tu.ddp import master_only
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # model parameters
        parser.add_argument('--model', type=str, default='monet',
        ...
        parser.add_argument('--max_val_dataset_size', type=int, default=float("inf"),
                            help='Maximum number of samples allowed per dataset. If the dataset directory contains more than max_dataset_size, only a subset is loaded.')
        ...
        parser.add_argument('--wandb_prefix', type=str, default='', help='wandb run name prefix')
        parser.add_argument('--nearest_valid', action='store_true',
                            help='refine translation predictions to nearest valid points')
        parser.add_argument('--unstrict_load', action='store_true', help='Load state dict with strict=False')
    ...
    def print_options(self, opt):
        ...
        file_name = os.path.join(expr_dir, f"{opt.phase}_opt.json")
        if self.isTrain:
            while os.path.exists(file_name):
                file_name = os.path.splitext(file_name)[0] + '_cont.json'
        print(f"saving options to {file_name}")
