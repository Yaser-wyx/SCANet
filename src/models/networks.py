...
class ResidualBlock3D(nn.Module):
    def __init__(self, in_channels, out_channels, num_conv_layers=2, stride=1):
        ...
        for _ in range(1, num_conv_layers):
            ...
            layers.append(nn.BatchNorm3d(out_channels))
            if _ < num_conv_layers - 1:
                layers.append(nn.ReLU(inplace=True))
        self.conv_layers = nn.Sequential(*layers)
        self.stride = stride

        # 如果输入通道数与输出通道数不一致，需要添加一个卷积层进行匹配
        self.match_dimensions = nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
        self.bn_match = nn.BatchNorm3d(out_channels)
    ...
    def forward(self, x):
        ...
        # 如果输入通道数与输出通道数不一致，匹配尺寸
        if x.size(1) != out.size(1) or self.stride != 1:
            residual = self.match_dimensions(x)
            residual = self.bn_match(residual)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_conv_layers=2, stride=1):
        ...
        self.bn_match = nn.BatchNorm2d(out_channels)
...
def ndc_meshgrid(size,device="cpu"):
    ...
    z = torch.linspace(0.025, 0.055, size[-1],device=device)
...
def occ_inds2fmap(occ_inds, overlap=True, title=None):
    imgs = occ_inds.float()
    ...
    for i in range(imgs.shape[0]):
        if imgs[i].shape[0] > 1:
            ...
            img.save(debug_dir + f'{str(i).zfill(3) if title is None else title}.png')
        if overlap:
