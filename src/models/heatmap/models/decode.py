...
def lego_decode_bid_shape_cond(heat, rot, bid_counter, reg=None, trans=None):
    batch = len(heat)
    height, width = heat[0].shape[-2], heat[0].shape[-1]
    heat = [_nms(h) for h in heat]

    detections = []
    for i in range(batch):
        batch_detections = []
        for j, (bid, ct) in enumerate(bid_counter[i]):
            h = heat[i][j].reshape(-1)
            this_scores, this_inds = torch.topk(h, k=ct)
            ys = (this_inds / width).int().float()
            xs = (this_inds % width).int().float()
            if reg is not None:
                xs += reg[i].reshape(reg[i].shape[0], 2, -1)[j, 0, this_inds]
                ys += reg[i].reshape(reg[i].shape[0], 2, -1)[j, 1, this_inds]
            else:
                xs += 0.5
                ys += 0.5
            rot_shape = rot[i].shape
            this_rot = rot[i].reshape(rot_shape[0], rot_shape[1], -1)[j, :, this_inds].argmax(0)

            if trans is not None:
                this_trans = trans[i].reshape(trans.shape[0], trans.shape[1])[j, :, this_inds]

            kps = torch.stack([xs, ys],
                              dim=1)
            from src.datasets.definition import gdef
            kps = (kps * gdef.down_ratio).round()

            clses = torch.ones_like(this_scores) * bid
            if trans is None:
                this_detections = torch.cat([kps, this_scores[:, None], clses[:, None], this_rot[:, None]], dim=-1)
            else:
                this_detections = torch.cat([kps, this_scores[:, None], clses[:, None], this_rot[:, None], this_trans],
                                            dim=-1)
            batch_detections.append(this_detections)
...
# 该代码实现了一个LEGO模型的解码函数，用于将模型输出的热图、旋转信息、标签、掩码等信息解码成目标检测结果。
def lego_decode_bid(heat, rot, bids_all, masks, pred_mask, reg=None, trans=None, predict_bid=None):
    batch, cat, height, width = heat.size()  # 获取输入热图的尺寸信息

    # 对热图进行非极大值抑制（NMS）
    heat = _nms(heat)
    if heat.shape[1] != 1:  # 如果热图通道数不为1
        heat = torch.softmax(heat, dim=1)  # 对热图进行softmax操作

    from collections import Counter
    detections = []  # 存储检测结果
    for i in range(batch):  # 遍历batch
        bids = bids_all[i][masks[i].bool()].detach().cpu().tolist()  # 获取当前batch的标签信息
        bids_ct = Counter(bids)  # 统计标签出现的次数
        batch_detections = []  # 存储当前batch的检测结果

        for bid, ct in bids_ct.items():  # 遍历标签及其出现次数
            h = heat[i, bid, :, :].reshape(-1)  # 获取对应标签的热图信息
            this_scores, this_inds = torch.topk(h, k=ct)  # 获取top-k的得分和索引
            ys = (this_inds / width).int().float()  # 计算关键点的y坐标
            xs = (this_inds % width).int().float()  # 计算关键点的x坐标
            if reg is not None:  # 如果存在回归信息
                xs += reg.reshape(batch, 2, -1)[i, 0, this_inds]  # 根据回归信息修正x坐标
                ys += reg.reshape(batch, 2, -1)[i, 1, this_inds]  # 根据回归信息修正y坐标
            else:
                xs += 0.5  # 否则默认修正x坐标
                ys += 0.5  # 否则默认修正y坐标
            if rot is not None:  # 如果存在旋转信息
                this_rot = rot.reshape(batch, rot.size(1), -1)[i, :, this_inds].argmax(0)  # 获取最大旋转角度

            if predict_bid is not None:  # 如果存在预测标签信息
                this_bid = predict_bid.reshape(batch, predict_bid.size(1), -1)[i, :, this_inds].argmax(0)  # 获取预测标签

            if trans is not None:  # 如果存在仿射变换信息
                this_trans = trans.reshape(batch, 3, -1)[i, :, this_inds].permute(1, 0)  # 获取仿射变换信息
            if pred_mask is not None:
                mask_pred = pred_mask[i]
                mask_pred_decode = mask_pred.argmax(0)
                this_mask = (mask_pred_decode == bid + 1).float().flatten()[None].repeat(ct, 1)

            kps = torch.stack([xs, ys],
                              dim=1)  # 构建关键点坐标
            from src.datasets.definition import gdef  # 导入数据集定义模块
            kps = (kps * gdef.down_ratio).round()  # 根据下采样比例对关键点坐标进行四舍五入取整

            if predict_bid is None:  # 如果不存在预测标签信息
                clses = torch.ones_like(this_scores) * bid  # 则类别为当前标签
            else:
                clses = this_bid  # 否则类别为预测标签
            to_cat = [kps, this_scores[:, None], clses[:, None]]  # 待拼接的信息列表，keypoint值（2d），所属类别的可能性以及类别信息
            if rot is not None:  # 如果存在旋转信息
                to_cat.append(this_rot[:, None])  # 添加旋转信息
            if trans is not None:  # 如果存在仿射变换信息
                to_cat.append(this_trans)  # 添加仿射变换信息
            if pred_mask is not None:
                to_cat.append(this_mask)  # 添加mask信息
            this_detections = torch.cat(to_cat, dim=-1)  # 拼接得到检测结果
            batch_detections.append(this_detections)  # 将当前检测结果添加到batch结果中
        batch_detections = torch.cat(batch_detections, dim=0)  # 将当前batch的检测结果拼接
        detections.append(batch_detections)  # 将当前batch的检测结果添加到总结果中
    # 返回检测结果，检测结果为一个list，list大小为batch大小，每一个item为k*(5+1)，
    # 其中k代表该步骤用到的组件个数，5+1表示[keypoint值（2d），所属类别的可能性, 类别信息, 旋转信息，预测的mask (flatten)]
    return detections



def lego_decode(heat, rot, reg=None, K=100):
    batch, cat, height, width = heat.size()
    ...
    rot = _transpose_and_gather_feat(rot, inds)
    rot = rot.argmax(dim=-1, keepdims=True)
    ...
    from src.datasets.definition import gdef
    kps = (kps * gdef.down_ratio).round()
    detections = torch.cat([kps, scores, clses, rot], dim=2)
