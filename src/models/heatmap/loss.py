...
class KPLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        hm_loss, rot_loss, off_loss = 0, 0, 0  # 初始化heatmap损失、旋转损失和偏移损失为0
        if 'bid' in batch:
            bid_loss = 0

        if opt.predict_trans:
            trans_loss = 0
        for s in range(opt.num_stacks):  # 遍历堆叠的每一个Hourglass Network的输出
            output = outputs[s]
            output['hm'] = _sigmoid(output['hm'])  # 对heatmap输出进行sigmoid激活
            hm_pred_list = output['hm'].detach().cpu().numpy()[0]
            # for idx, hm_pred in enumerate(hm_pred_list):
            #     plt.imshow(hm_pred)
            #     plt.axis('off')
            #     plt.title(f"pred_{idx}")
            #     plt.show()

            hm_gt_list = batch['hm'].cpu().numpy()[0]
            # for idx, hm_gt in enumerate(hm_gt_list):
            #     plt.imshow(hm_gt)
            #     plt.axis('off')
            #     plt.title(f"gt_{idx}")
            #     plt.show()
            hm_loss += self.crit(output['hm'], batch['hm']) / opt.num_stacks  # 计算loss（focal loss），并正则化
...
class MaskLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            output = outputs[s]
            # batch_mask = batch['mask'].cpu().numpy()
            # plt.imshow(batch_mask[0])
            # plt.axis('off')
            # plt.title("GT")
            # plt.show()
            # pred_mask = output['mask'].detach()
            # pred = torch.argmax(pred_mask[0], dim=0).cpu().numpy()
            # plt.imshow(pred)
            # plt.axis('off')
            # plt.title("pred")
            # plt.show()
