...
from src.lego.utils.inference_utils import compute_2d_kp_to_3d_translation, compute_candidates_iou, recompute_conns
from src.lego.utils.inference_utils import dist_matching_greedy
from src.tu.ddp import get_distributed_model, master_only
from src.util.util import transform_points_screen
...
class HourglassShapeCondModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netHG = LegoHourGlass(
            ...
            occ_fmap_size=opt.occ_fmap_size,  # 特征图大小
            shape_condition=True,
            brick_emb=opt.brick_emb_dim,  # brick编码的特征维度
            one_hot_brick_emb=opt.one_hot_brick_emb,
            num_bricks_single_forward=opt.num_bricks_single_forward,
            # the maximum number of distinct components considered in MEPNet in single forward
    ...
    def forward(self):
        ...
        if self.opt.grad_ckpt:
            ...
            self.outputs, self.loss_sum, loss_dict = self.netHG(self.img, self.occ_prev,
                                                                self.azims, self.elevs, self.obj_q, self.obj_scales,
                                                                self.obj_centers,
                                                                target, self.brick_occs, self.bids)  # 第一阶段
        ...
        if not self.netHG.training:
            ...
            with torch.no_grad():
                if self.opt.num_bricks_single_forward == 1:
                    ...
                    bids_tmp = torch.zeros_like(self.bids_flat)  # 初始化临时砖块张量
                    ...
                    detections = lego_decode_bid(self.outputs[-1]['hm'],
                                                 ...
                                                 self.outputs[-1]['mask'],
                                                 self.outputs[-1][
                                                     'reg'])  # 检测结果为一个list，list大小为batch大小，每一个item为k*5，其中k代表该步骤用到的组件个数，5表示[keypoint值（x,y），所属类别的可能性, 类别信息, 旋转信息]
                    for i in range(len(detections)):
                        # map back to the bid
                        for j in range(detections[i].shape[0]):
                            detections[i][j, 3] = self.bid_counter[i][int(detections[i][j, 3].item())][0]  # 更新检测结果的砖块编号
                ...
                for i in range(len(self.hms)):
                    ...
                    idxs_pred, dists_matched = dist_matching_greedy(kps_pred, kps_target_list)  # 执行匈牙利匹配
                    detection_matched = detections[i][idxs_pred]  # 获取匹配的检测结果
                    reorder_idxs_list.append(idxs_pred)  # 添加重排序索引
                    # 计算kp，mask以及rotation的准确率
                    ...
                    output_mask_shape = self.outputs[-1]['mask'].shape[2:]
                    detection_mask_data = detection_matched[:, 5:]
                    result_batch, detection_dim = detection_mask_data.shape
                    detection_mask = detection_mask_data.reshape(result_batch, *output_mask_shape)
                    self.detections.append({
                        'kp': detection_matched[:, :2],
                        'bid': detection_matched[:, 3],
                        'rot': detection_matched[:, 4],
                        'mask': detection_mask,  # 扩展一个维度
                    })

                # 3D Pose inference，第二阶段
                if self.opt.load_conns and self.opt.load_bricks:
                    ...
                    for i in range(len(self.detections)):
                        ...
                        for j in range(num_objs):
                            ...
                            bid_this = self.detections[i]["bid"][j].long().item()  # 获取当前brick_id
                            if bid_this < 0:
                            ...
                            if self.op_types[i][j].item() == 2:  # pass
                                ...
                                if not self.opt.predict_masks or not self.opt.search_ref_mask or cbrick is None:
                                    ...
                                    from src.bricks.brick_info import cbrick_highest_brick
                                    import itertools
                                    if bid_this < 0:
