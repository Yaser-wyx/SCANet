...
from .utils import elevate_pts, offset_pts
...
class BasicBrickInfo:
    def __init__(self,
                 bid: str,  # 砖块的唯一标识符
                 # List of 3d points.
                 occ_pts: List[np.array],  # 占据点的列表
                 stud_pts: List[np.array],  # 凸起点的列表
                 astud_pts: List[np.array],  # 反凸起点的列表
                 # For detecting base collision.
                 base_contour: List[np.array],  # 用于检测基座碰撞的轮廓点列表
                 height: int,  # 砖块的高度
                 rotations: List[int],  # 可用的旋转角度列表
                 ):
        self.bid = bid
        ...
        self.bbox_xyz = get_brick_bbox_xyz(self.occ_pts)  # 获取砖块的边界框坐标

    def get_info(self):
        return self.occ_pts, self.stud_pts, self.astud_pts, self.base_contour  # 返回砖块的占据点、凸起点、反凸起点和基座轮廓
...
def get_brick_info(brick, grid_size, expand_stud=False):
    ...
    """
    ps = get_brick_info_raw(brick.brick_type)  # 获取原始砖块信息
    ps_transformed = []  # 转换后的砖块信息
    for i in range(len(ps)):
        p = ps[i]
        if p:
            p_round = transform_points_round(p, brick.transform_matrix)  # 对砖块信息进行转换和取整
            ps_transformed.append(p_round)
        else:
            ps_transformed.append([])
    # expand occupancy points 2x
    brick_pts = ps_transformed[0]  # 砖块占用点
    brick_pts_exp = set()  # 扩展后的砖块占用点集合
    ...
    if (brick_pts > xyz_max).any() or (brick_pts < xyz_min).any():  # 如果砖块超出网格范围，则返回空列表
        return [None] * 4

    if expand_stud:  # 如果需要扩展插销，默认不扩展
    ...
    for offset_x in [-0.25, 0.25]:
        for offset_y in [-0.25, 0.25]:
            for offset_z in [-0.25, 0.25]:
                for p in brick_pts:  # 遍历砖块占用点
                    p_offset = p + [offset_x, offset_y, offset_z]  # 计算偏移后的砖块占用点
                    brick_pts_exp.add(tuple(p_offset))  # 将偏移后的砖块占用点添加到扩展后的砖块占用点集合
                if expand_stud:
                    for p in stud_pts:  # 遍历插销点
                        p_offset = p + [offset_x, offset_y, offset_z]
                        stud_pts_exp.add(tuple(p_offset))  # 将偏移后的插销点添加到扩展后的插销点集合
                    for p in astud_pts:  # 遍历辅助插销点
                        p_offset = p + [offset_x, offset_y, offset_z]
...
def get_brick_enc_pc_(bid):
    grid_size = [65, 65, 65]
    brick = Brick(bid, [0, 0, 0], [1, 0, 0, 0])
    occ_pts, studs_pts, astuds_pts, _, = get_brick_info(brick, grid_size, expand_stud=True)
    return {'occ_pts': occ_pts, "studs_pts": studs_pts}


def brick_pc2voxel(bid, pc_map, position=None, rotation=None, return_pc=False):
    stud_id, bid2id, id2value = get_brick_enc_voxel_info()
    occ_pts, studs_pts, extra_point, extra_point_value = \
        pc_map['occ_pts'], pc_map['studs_pts'], pc_map['extra_point'], pc_map['extra_point_value']
    transform_mat = None
    ...
    if return_pc:
        occ_voxel, occ_pts = pts2voxel(occ_pts, id2value[bid2id[bid]], return_pts=True)
    else:
        occ_voxel = pts2voxel(occ_pts, id2value[bid2id[bid]])
        occ_pts = np.stack(list(occ_pts))

    if extra_point is not None:
        extra_point = np.asarray(extra_point, float)
        if len(extra_point.shape) < 2:
            extra_point = np.expand_dims(extra_point, axis=0)
        if transform_mat is not None:
            extra_point = transform_points_round(extra_point, transform_mat)
        if return_pc:

            extract_voxel, extra_point = pts2voxel(extra_point, extra_point_value, return_pts=True)

        else:
            extract_voxel = pts2voxel(extra_point, extra_point_value)

        occ_voxel += extract_voxel
        occ_pts = np.concatenate((occ_pts, extra_point))
    ...
    if studs_pts:
        ...
        if return_pc:
            stud_voxel, studs_pts = pts2voxel(studs_pts, id2value[stud_id], return_pts=True)

            return occ_voxel + stud_voxel, np.concatenate((occ_pts, studs_pts))
        else:
            stud_voxel = pts2voxel(studs_pts, id2value[stud_id])
            return occ_voxel + stud_voxel
    else:
        if return_pc:
            return occ_voxel, occ_pts
        else:
            return occ_voxel


def pts2voxel(pts, value, voxel_translation=None, voxel_size=None, return_pts=False):
    if voxel_size is None:
        voxel_size = [130, 130, 130]
    if voxel_translation is None:
        voxel_translation = [-65, -65 // 4, -65]

    pts = np.stack(list(pts))
    pts = (pts * 2 - 0.5)
    pts = pts.round().astype(np.int)
    ...
    try:
        ...
        warning("The voxel exceeds the grid size!")
        pts = pts[(pts >= 0).all(axis=1) & (pts < np.array(voxel_size)).all(axis=1)]
...
def get_brick_enc_voxel_(bid, position=None, rotation=None, extra_point=None, extra_point_value=None, use_id=False):
    ...
    brick = Brick(bid, [0, 0, 0], [1, 0, 0, 0])
    occ_pts, studs_pts, _, _, = get_brick_info(brick, grid_size, expand_stud=True)
...
def get_cbrick_enc_pc(cbrick):
    pc_map = {'occ_pts': [], "studs_pts": []}

    for i, b in enumerate(cbrick.bricks_raw):
        sub_pc_map = get_brick_enc_pc_(b.brick_type)
        pc_map['occ_pts'].extend(sub_pc_map['occ_pts'])
        pc_map['studs_pts'].extend(sub_pc_map['studs_pts'])
...
class Brick:
    def __init__(self, brick_type, position, rotation, state_flag=None):
        '''
...
from src.bricks.utils import line_area
...
class BricksPC:
    ...
    def get_occ_with_rotation(self):
        ...
        # 获取张量中为 True 的元素的坐标
        # indices = np.where(rot_occ)

        # 将坐标存储为点云数据
        # point_cloud = np.column_stack(indices)
        ...
        # 可视化点云数据
        # ax.scatter(occ_pos[:, 0], occ_pos[:, 1], occ_pos[:, 2], s=5)
...
class CBrick:
    ...
    def __init__(self, bricks_pc: BricksPC, position, rotation, state_flag=None):
...
def dict_to_cbrick(b_d, state_flag=None, reset_pose=False, no_check=False):
    '''Reset_pose is used when you want a canonical cbrick.'''
    b = BricksPC.from_dict(b_d['canonical_state']['bricks_pc'], no_check=no_check)
    if not b:
        return None
    if not reset_pose:
        return CBrick(b, b_d['brick_transform']['position'], b_d['brick_transform']['rotation'], state_flag)
    else:
        return CBrick(b, [0, 0, 0], [1, 0, 0, 0], state_flag)
